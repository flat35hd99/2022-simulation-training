...
void set_diameter(double *a) {
  for (int i = 0; i < Np; i++) a[i] = 1.0 + 0.1 * gaussian_rand();
}

void p_boundary(double (*x)[dim]) {
  int i, j;
  for (i = 0; i < Np; i++)
    for (j = 0; j < dim; j++) x[i][j] -= L * floor(x[i][j] / L);
}
...
void calc_force(double (*x)[dim], double (*f)[dim], double *a) {
  ...
  ini_array(f);
#pragma omp parallel for default(none) private(i, j, dx, dy, dr2, dUr, w2, w6, \
                                               w12, aij) shared(x, f, a)
  for (i = 0; i < Np; i++)
    for (j = 0; j < Np; j++) {
      if (i < j) {
        dx = x[i][0] - x[j][0];
