...
void set_diameter(double *a) {
  for (int i = 0; i < Np; i++) a[i] = 1.0 + polydispersity * gaussian_rand();
}

void p_boundary(double (*x)[dim], int i) {
  for (int j = 0; j < dim; j++) x[i][j] -= L * floor(x[i][j] / L);
}
...
void mc(double (*x)[dim], double *a, double *U, double temp0, int *count) {
  ...
  if (p > 1. / exp((U[i] - U0) / temp0)) {
    *count += 1;
    ...
    for (int k = 0; k < dim; k++) x[i][k] -= dr[k];
    U[i] = U0;
  }
  p_boundary(x, i);
}
...
int main() {
  ...
  std::cout << "rejection rate = " << (double)count / mcstep_max / Np
            << std::endl;
  return 0;
}
